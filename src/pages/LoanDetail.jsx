  ...
  const generateReceipt = (payment) => {
    const receiptHtml = `
      <html><head><style>
        ...
        <div class="row" style="color:#3b82f6;"><span>Capital:</span><strong>RD$ ${(loan?.amount || 0).toFixed(2)}</strong></div>
        <div class="row" style="color:#d4a533;"><span>Réditos (Interés):</span><strong>RD$ ${(loan?.total_interest || 0).toFixed(2)}</strong></div>
        ${mora > 0 ? `<div class="row" style="color:#ef4444;"><span>Mora Acumulada:</span><strong>RD$ ${mora.toFixed(2)}</strong></div>` : ''}
        <div class="row" style="font-weight:bold;"><span>Pago Total Adeudado:</span><strong>RD$ ${((loan?.remaining_balance || 0) + mora).toFixed(2)}</strong></div>
        <div class="row" style="color:#10b981;font-weight:bold;"><span>Pago Recibido:</span><strong>RD$ ${payment.amount?.toFixed(2)}</strong></div>
  ...
  const downloadReceiptAsImage = async (payment) => {
    ...
    container.innerHTML = `
      <div style="font-family:Arial,sans-serif;width:400px;margin:0 auto;padding:20px;background:white;">
        ...
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #eee;color:#3b82f6;"><span>Capital:</span><strong>RD$ ${(loan?.amount || 0).toFixed(2)}</strong></div>
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #eee;color:#d4a533;"><span>Réditos (Interés):</span><strong>RD$ ${(loan?.total_interest || 0).toFixed(2)}</strong></div>
        ${mora > 0 ? `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #eee;color:#ef4444;"><span>Mora Acumulada:</span><strong>RD$ ${mora.toFixed(2)}</strong></div>` : ''}
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #eee;font-weight:bold;"><span>Pago Total Adeudado:</span><strong>RD$ ${((loan?.remaining_balance || 0) + mora).toFixed(2)}</strong></div>
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #eee;color:#10b981;font-weight:bold;"><span>Pago Recibido:</span><strong>RD$ ${payment.amount?.toFixed(2)}</strong></div>
